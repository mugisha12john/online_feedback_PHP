  ...
  >
    <div class="bg-white shadow-lg rounded-2xl w-full max-w-md p-8 relative">
      ...
      <form  method="POST" class="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            name="email"
            required
            placeholder="e.g., example@admin.rw"
            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500"
          />
        </div>
        <div>
      ...
      <p class="text-center text-gray-600 text-sm mt-4">
        ...
        <a href="login_admin.php" class="text-orange-600 font-medium hover:underline">
          Login
        </a>
      </p>
    </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($password !== $confirm_password) {
        ...
        if ($result->num_rows > 0) {
            ...
            $insertQuery = "INSERT INTO admins (username, email, password) values ('$firstname $lastname', '$email', '$hashedPassword')";
            if ($conn->query( $insertQuery)) {
   

                echo "<script>alert('Registration successful! You can now log in.'); window.location.href='login_admin.php';</script>";
            } else {
